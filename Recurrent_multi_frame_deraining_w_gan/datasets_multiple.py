...
class MultiFramesDataset(data.Dataset):
    ...
    def __init__(self, opts, dataset, mode):
        ...
        for video in videos:
            ...
            input_dir = os.path.join(self.opts.data_dir, self.mode, "input", video)
    ...
    def __getitem__(self, index):
        ...
        input_dir = os.path.join(self.opts.data_dir, self.mode, "input", video)
        process_dir = os.path.join(self.opts.data_dir, self.mode, "processed", video)
        ...
        for t in range(T+1, T + self.opts.sample_frames+1):
            frame_i.append(utils.read_img(os.path.join(input_dir, "%d.png" % t)))
            frame_p.append(utils.read_img(os.path.join(process_dir, "%d.png" % t)))
