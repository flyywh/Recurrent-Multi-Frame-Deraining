...
if __name__ == "__main__":
    ...
    parser.add_argument('-data_dir',        type=str,     default='data_practical',           help='path to data folder')
    ...
    list_filename = os.path.join(opts.list_dir, "rain_removal_real_test.txt")
    ...
    for v in range(len(video_list)):
        ...
        for t in range(1+3, len(frame_list)-3+3):
            frame_i1 = utils.read_img(os.path.join(input_dir, "%05d.jpg" %(t))) 
            frame_i2 = utils.read_img(os.path.join(input_dir, "%05d.jpg" %(t+1)))
            frame_i3 = utils.read_img(os.path.join(input_dir, "%05d.jpg" %(t+2)))
            frame_i4 = utils.read_img(os.path.join(input_dir, "%05d.jpg" %(t+3)))
            frame_i5 = utils.read_img(os.path.join(input_dir, "%05d.jpg" %(t+4)))
            ...
            with torch.no_grad():
                ...
                for s in range(h, H_sc):
                    frame_i1_new[:,:,s,:] = frame_i1_new[:,:,2*h-1-s,:]
                    frame_i2_new[:,:,s,:] = frame_i2_new[:,:,2*h-1-s,:]
                    frame_i3_new[:,:,s,:] = frame_i3_new[:,:,2*h-1-s,:]
                    frame_i4_new[:,:,s,:] = frame_i4_new[:,:,2*h-1-s,:]
                    frame_i5_new[:,:,s,:] = frame_i5_new[:,:,2*h-1-s,:]

                for s in range(w, W_sc):
                    frame_i1_new[:,:,:,s] = frame_i1_new[:,:,:,2*w-1-s]
                    frame_i2_new[:,:,:,s] = frame_i1_new[:,:,:,2*w-1-s]
                    frame_i3_new[:,:,:,s] = frame_i1_new[:,:,:,2*w-1-s]
                    frame_i4_new[:,:,:,s] = frame_i1_new[:,:,:,2*w-1-s]
                    frame_i5_new[:,:,:,s] = frame_i1_new[:,:,:,2*w-1-s]
            ...
            output_filename = os.path.join(output_dir, "%05d_s2.png" %(t+2))
            utils.save_img(frame_res3_s2, output_filename)
            output_filename = os.path.join(output_dir, "%05d_s1.png" %(t+2))
            utils.save_img(frame_res3_s1, output_filename)
            output_filename = os.path.join(output_dir, "%05d_input.png" %(t+2))
