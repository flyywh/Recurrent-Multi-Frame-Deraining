...
class MultiFramesRealDataset(data.Dataset):

    def __init__(self, opts, dataset, mode):
        super(MultiFramesRealDataset, self).__init__()
        ...
        for video in videos:
            ...
            input_dir = os.path.join(self.opts.data_real_dir, self.mode, video)
    ...
    def __getitem__(self, index):
        ...
        input_dir = os.path.join(self.opts.data_real_dir, self.mode, video)
        process_dir = os.path.join(self.opts.data_real_dir, self.mode, video)
        ...
        for t in range(T+1, T + self.opts.sample_frames+1):
            ...
            frame_p.append(utils.read_img(os.path.join(process_dir, "%05d.jpg" % t)))
