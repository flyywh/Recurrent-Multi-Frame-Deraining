...
def mask_gen(frame_i):
    [b, c, h, w] = frame_i.shape
    zero_mask = torch.zeros(b, c, h, w).cuda()
    random_mask = torch.rand(b, c, h, w).cuda()
    random_mask_ori = random_mask.clone()

    random_mask[random_mask_ori>0.2] = 0
    random_mask[random_mask_ori<=0.2] = 1

    random_mask = random_mask[:, 0:1, :, :]
    random_mask = torch.cat((random_mask, random_mask, random_mask), 1)

    return random_mask, zero_mask
...
if __name__ == "__main__":
    ...
    if opts.solver == 'SGD':
        ...
        optimizer = optim.Adam([ \
                                {'params': multi_model_res.parameters(), 'lr': 1e-4}, \
    ...
    while multi_model_res.epoch < opts.epoch_max:
        ...
        for iteration, batch in enumerate(data_haze_loader, 1):
            ...
            for t in range(0, opts.sample_frames-4):
                ...
                Multi_loss2 += -criterion_ssim(frame_res3_s2, frame_g3.detach()) 
               
            overall_loss =  Multi_loss + Multi_loss2
            ...
            if opts.w_RECT > 0:
                ...
                loss_writer.add_scalar('Multi_loss2', Multi_loss2.item(), total_iter)
                info += "\t\t%25s = %f\n" %("Multi_loss2", Multi_loss2.item())
